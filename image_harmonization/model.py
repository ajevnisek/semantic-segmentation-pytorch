...
class UpsampleConcat(nn.Module):
    ...
    def forward(self, x1, x2):
        x = torch.cat((x1, x2), 1)
        return F.relu(self.bn(self.conv2d(F.interpolate(x, scale_factor=self.scale_factor, mode="bilinear", align_corners=True))),
                      inplace=False)


class MaskGeneratingNetwork(nn.Module):
    def __init__(self, input_dim=3):
        super(MaskGeneratingNetwork, self).__init__()
        self.Down1 = ConvBlock(input_dim, 64, stride=(2, 2))
        ...
        self.output = nn.Conv2d(16, 1, kernel_size=(3, 3), padding=1)
